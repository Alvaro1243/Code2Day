...
function showGallery(currarray) {
  ...
  for(var i=0; i<currarray.length; i++) {
    ...
    document.getElementById("card").innerHTML += ` 
      ...
      <a class = "especial" href="./detallescurso.html?title=${encodeURIComponent(currarray[i].title)}&image=${encodeURIComponent(currarray[i].image)}&rating=${currarray[i].rating}&description=${encodeURIComponent(currarray[i].description)}&date=${currarray[i].date}&inscrito=${0}" alt="not found"><img src="${currarray[i].image}" alt="Course Image" longdesc=""/></a>
